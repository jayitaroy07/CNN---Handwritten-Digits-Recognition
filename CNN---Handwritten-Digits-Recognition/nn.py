...
class NN(object):
    ...
    def load_weight(self, path):
        ...
        try:
            self.weights = np.load(os.path.join(path, 'weights.npy'), allow_pickle=True, encoding='latin1').item()
        except:
            raise Exception('Not able to load Network weights')

        try:
            self.biases = np.load(os.path.join(path, 'biases.npy'), allow_pickle=True, encoding='latin1').item()
        except:
            raise Exception('Not able to load Network biases')

        try:
            self.batch_norm_params = np.load(os.path.join(path, 'bn_params.npy'),
    ...
    def train(self):
        ...
        for i in range(self.nepochs):
            print("i", i, self.nepochs)
            for j in range(int(it_per_epoch)):
                print("j", j, it_per_epoch)
